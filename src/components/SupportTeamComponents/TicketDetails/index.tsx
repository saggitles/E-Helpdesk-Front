...
import { faSpinner, faCheckCircle, faTimesCircle, faCircleNotch, faBan } from '@fortawesome/free-solid-svg-icons';
...
type JiraTicket = {
  ...
  CreationDate?: string;  // Optional string field, assuming date is stored as a string
  Description?: string;  // Optional string field
  ProjectKey?: string;  // Optional string field
  ProjectName?: string;  // Optional string field
  ProjectType?: string;  // Optional string field
  Status?: string;  // Optional string field
  StatusCategory?: string;  // Optional string field
...
const fetchComments = async (token: string, ticketId: string) => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tickets/${ticketId}/comments`, {
      headers: {
...
export const TicketDetails = () => {
  ...
  const { Title, Description, Supported, category, priority, Reporter, Platform, Email, Contact, Companyname, Status, customer, department, site, create, update, IDTicket, Dealer, VehicleID, JiraTicketID, Solution, incidentDate, TicketNumber,  openSince} = router.query;
  const [commentContent, setCommentContent] = useState<string>(''); // Assuming you have an input for comment content
  const [selectedAction, setSelectedAction] = useState<string>('unassigned');
  const [isPopJiraVisible, setPopJiraVisibility] = useState(false);
  const [showConfirmation, setShowConfirmation] = useState(false);
  const [selectedProjectForEscalation, setSelectedProjectForEscalation] = useState('');
  const [deleteConfirmVisible, setDeleteConfirmVisible] = useState(false);
  const [selectedFileIdToDelete, setSelectedFileIdToDelete] = useState<string | null>(null);
  const [jiraTicket, setJiraTicket] = useState<JiraTicket | null>(null);
  const [jiraTicketOfficial, setJiraTicketOfficial] = useState<JiraTicketOfficial | null>(null);
  const [isSolutionPopupVisible, setSolutionPopupVisibility] = useState(false);
    ...
    if (createdAtString) {
      ...
      if (days === 1) {
        ...
        if (remainingHours === 1) {
          ...
          setTimeSinceCreated(`${days} days and ${remainingHours} hours ago`);
        } else {
  ...
  useEffect(() => {
    const fetchAssigneeId = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/tickets/${ticketId}/assigneduser`, config);
  ...
  useEffect(() => {
    const fetchUsersData = async () => {
      try {
        ...
        const response = await axios.get<User[]>(`${process.env.NEXT_PUBLIC_API_URL}/api/users`, config);
      ...
      try {
        ...
        if (urlNew) {
          ...
          if (jiraIssueNumber) {
            ...
            const respuestaJiraDetails = axios.get(apiUrl, config)
                  .then(response => {
                      console.log("Respuesta del servidor para el issue de Jira:", response.data);
                      setJiraTicketOfficial(response.data)

                  })
                  .catch(error => {
                      console.error("Error al hacer la solicitud al servidor local:", error);
                  });
    ...
    if (typeof JiraTicketID === 'string') {
      const IDTicketNumber = parseInt(JiraTicketID, 10);
  ...
  } else if (Array.isArray(IDTicket)) {
    ...
    const fetchAssigneeId = async () => {
      try {
        ...
        if (assigneeId) {
          ...
          const userResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/users/${assigneeId}`, config);
  ...
  const getSelectedAssigneeName = () => {
    const selectedAssigneeUser = users.find(user => user.IDUser.toString() === selectedAssignee);
    return selectedAssigneeUser ? `${selectedAssigneeUser.FirstName} ${selectedAssigneeUser.LastName}` : "N/A";
  };
  ...
  const handleSaveClick = async () => {
    ...
    if (ticket.Status === 'Done') {
      ...
      if (createdAtString) {
        ...
        const createdAtDate = new Date(Date.UTC(year, month - 1, day, hours, minutes, seconds));
        const currentTime = new Date();
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tickets/${IDTicket}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleInputChange = (key: string, value: string | string[] | undefined) => {

    const stringValue = Array.isArray(value) ? value.join(', ') : value;
    setEditedValues((prevValues) => ({ ...prevValues, [key]: stringValue || '' }));
  ...
  const confirmDeleteAttachment = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/tickets/${ticketId}/attachments/${selectedFileIdToDelete}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const escalateToDeveloper = async (ticketData: TicketData) => {
    try {
      ...
      const requestData = {
        ...
        idTicket: parseInt(IDTicket as string, 10)
      };
  ...
  const handleAddComment = async (content: string) => {
    console.log('Ticket ID:', IDTicket, 'Content', content);
    try {
      const token = localStorage.getItem('accessToken');
      ...
      const userResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/users/?email=${userEmail}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  if (create) {
    ...
    const createDate = new Date(create as string);

    // Formatear la fecha y la hora
    formattedDate = createDate.toISOString().split('T')[0];
    formattedTime = createDate.toTimeString().split(' ')[0].substring(0, 5);
  }
  ...
  const AssignUser = async (e: React.FormEvent<HTMLFormElement> | ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tickets/assign/${router?.query?.IDTicket}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        console.log(`Usuario ${userIdToAssign} asignado al ticket con éxito`);
      } else {
  ...
  const handleSolutionChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    <>
      <div className="bg-teal-50 min-h-screen">
        <div className="py-10 px-14 overflow-y-auto max-h-fit">
          <h1 className='text-gray-600 text-center text-3xl mb-8'>
            Welcome to <span className="text-[#14b8a6] font-bold">Ticket Number: {router?.query?.IDTicket}</span>
          </h1>
          ...
          <div className='flex justify-between'>
              ...
              <div className=''>
                ...
                <div className='flex flex-wrap gap-x-5 pb-4'>
                  {attachments.map((attachment, index) => (
                    <div key={index} className='relative w-1/6 border border-teal-100 flex flex-col items-center mt-3'>
                      <div className='my-3'>
                        <FontAwesomeIcon size='2x' icon={faImages} className="text-[#14b8a6]" />
                      </div>
                      <p>{/*@ts-ignore*/}
                      ...
                      <div className='bg-gray-200 w-full text-center px-3'>
                        ...
                        <button className='absolute top-0 right-0 pr-1' onClick={() => 
                          // @ts-ignore
                ...
                <div className='flex flex-col space-y-10 ml-7 mt-5 mb-10'>
                  <div className='border border-gray-200 rounded-lg p-4 bg-gray-100'>
                    ...
                    <div className='bg-white rounded-md p-4 shadow-md'>
                      <p className='text-gray-700'>{Solution || 'No solution provided yet.'}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className='border border-gray-100 w-1/2 rounded h-1/2'>
              ...
              <div className='grid grid-cols-2' style={{ marginLeft: '15px' }}>
                  ...
                  {isEditing ? (
                    ...
                    >
                      <option value='' disabled>
                        {selectedAssignee ? `${selectedAssignee}` : "Assign to"}
                      </option>
                      {users.map((user) => (
                        <option key={user.IDUser} value={user.IDUser.toString()}>
                    ...
                    <div>
                      {assignee && (
                        <div>
                          <p className='text-md font-semibold'>{assignee.FirstName} {assignee.LastName}</p>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div className='flex flex-row mt-4' style={{ fontSize: '14px' }}>
                <div className='w-1/3'>
                ...
                <div className='w-2/3'>
                  <div className='w-2/3'>
                    {router.query.Status === 'Done' ? (
                      <p className='text-md font-semibold'>{router.query.openSince || 'N/A'}</p>
                    ) : (
                      <p className='text-md font-semibold'>{timeSinceCreated}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {Object.entries(router.query).map(([key, value]) => {
                if (key !== 'ticketNumber' && key !== 'IDTicket') {
                  return (
                    <div key={key} className="flex flex-row mt-4 items-center py-3 border-b border-teal-100 hover:bg-teal-50 transition-all duration-200">
                      <div className="w-1/3">
            ...
            <div className='border-2 border-gray-200 mt-20'>
              <div className='text-white px-5 py-3 text-xl font-semibold' style={{ 
                background: '#14b8a6',
                padding: '10px 50px',
                textAlign: 'center',
                display: 'inline-block',
              ...
              <div className='flex justify-between space-x-0 mb-5 mt-3 px-7 w-full'>
                {/* Jira Details Grid */}
                <div className='flex-grow' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '16px' }}>
                  <span className="text-lg mb-4" style={{ fontSize: '14px' }}>Ticket number</span>
                ...
                <div className='flex-grow' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '14px' }}>
                  <span className="text-lg mb-4" style={{ fontSize: '14px' }}>Escalated at</span>
                  <span className='text-base'>{jiraTicketOfficial?.fields?.project?.name || 'N/A'}</span>
                </div>

                <div className='flex-grow' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '14px' }}>
                  <span className="text-lg mb-4" style={{ fontSize: '14px' }}>Status</span>
                  <span className='text-base'>{jiraTicketOfficial?.fields?.status?.name || 'Unknown'}</span>
                </div>
                  ...
                  <div className="text-sm text-center">
                    {jiraTicketOfficial?.fields?.assignee ? (
                      <div className="group inline-block text-center">
                        <span className="block font-bold text-lg mb-1">Assignee</span>
                        <Image
                          src={jiraTicketOfficial.fields.assignee.avatarUrls["48x48"]}
                          alt="Avatar"
                          width={18}
                          height={18}
                          layout="fixed"
                        ...
                        <span className="absolute hidden group-hover:block w-32 bg-black text-white text-center text-xs rounded py-1 px-2 z-10 -bottom-8 left-1/2 transform -translate-x-1/2">
                          {jiraTicketOfficial.fields.assignee.displayName}
                        </span>
                      </div>
                    ) : (
      ...
      <div className='bg-gray-100 py-5 flex flex-row justify-between px-14 bottom-0 w-full'>
        <div className='flex items-center px-8 border border-gray-200 rounded-full text-sm text-[#14b8a6] hover:text-[#14b8a6]'>
          <p style={{ cursor: "pointer" }} onClick={() => handleRedirect("/support/tickets/pending")}>
            BACK
          </p>
        </div>
      ...
      {deleteConfirmVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to delete this attachment?</p>
            <div className="flex justify-end gap-4">
              <button
      ...
      {isPopJiraVisible && <PopJira onClose={handleClosePopJira} escalate={handlePopJiraEscalate} />}
