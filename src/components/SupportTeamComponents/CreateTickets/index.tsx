...
import ChatWidget from '@/components/chat/ChatWidget';  // Add this line
...
export const CreateTickets: React.FC<CreateTicketsProps> = ({ gmptCodeUpdated }) => {
 ...
 useEffect(() => {
  const saveUser = async () => {
    try {
      if (hasCheckedUser) return; // Prevent further execution if the user has already been checked
      const accessToken = localStorage.getItem('accessToken');

      // Ensure that 'user', 'accessToken', and 'user.email' are defined before proceeding
      if (user && accessToken && user.email) {
        const permissions = extractPermissionsFromJWT(accessToken);

        console.log("Access Token:", accessToken);
        console.log("Extracted Permissions:", permissions);

        // Define required permissions for the "Support_Team" role
        const supportTeamPermissions = [
          'create:comment',
          'create:ticket',
          'read:comment',
          'read:customer',
        ...
        const derivedRole = supportTeamPermissions.every((perm) =>
          permissions.includes(perm)
        )
          ? 'Support_Team'
          : 'Guest';

        console.log("Derived Role:", derivedRole);

        // Step 1: Ensure the role exists in the database and get the IDRole
        const roleID = await ensureRoleExists(derivedRole, accessToken);
        ...
        // Step 3: If the user does not exist, create a new user with the role ID
        if (!existingUserResponse.data || existingUserResponse.data.length === 0) {
          ...
          console.log('User already exists. Checking if role needs to be updated.');

          const existingUser = existingUserResponse.data[0];

          // Step 4: Update the user's role if necessary
          if (existingUser.UserRoleID !== roleID) {
            console.log(
              `User role has changed. Updating UserRoleID from ${existingUser.UserRoleID} to ${roleID}.`
            );

            // Update the user's role in the database
            const updateResponse = await axios.put(
              `${process.env.NEXT_PUBLIC_API_URL}/api/users/${existingUser.IDUser}`,
              {
                UserRoleID: roleID,
              },
              {
            ...
            console.log("Update User Role Response:", updateResponse.data);

            if (updateResponse.status === 200) {
              console.log('User role updated successfully:', updateResponse.data);
            } else {
              console.error('Failed to update user role:', updateResponse.statusText);
            }
          } else {
            console.log('User role is up-to-date. No changes needed.');
          }
        }

        setHasCheckedUser(true); // Mark that the user has been checked to prevent further execution
      }
    } catch (error) {
  ...
  const ensureRoleExists = async (roleName: string, accessToken: string): Promise<number> => {
    try {
      console.log("Checking if role exists:", roleName);

      const roleResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/roles?roleName=${roleName}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
      ...
      if (!roleResponse.data || roleResponse.data.length === 0) {
        ...
        if (createRoleResponse.status === 201) {
          ...
          return createRoleResponse.data.IDRole;
        } else {
          throw new Error('Failed to create role');
        }
      } else {
        return roleResponse.data[0].IDRole;
      }
    } catch (error) {
  ...
  const extractPermissionsFromJWT = (accessToken: string): string[] => {
  ...
  // Only run 'saveUser' if 'user' is defined and 'isLoading' is false
  if (!isLoading && user) {
    saveUser();
  }
}, [user, isLoading, hasCheckedUser]);
  ...
  useEffect(() => {
    const fetchDealers = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/dealers`, {
          headers: {
  ...
  const handleSelectChange = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/getCompanyFromDealer/${dealerId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
          ...
          try {
            ...
            if (axios.isAxiosError(error)) {
              if (error.response && error.response.status === 404) {
                alert('No encontrado: El vehículo con el ID especificado no fue encontrado.');
              } else {
  ...
  const executeFleetIQSearch = async (gmptCode: string) => {
    try {
      ...
      console.log("Ejecutando búsqueda de FleetIQ para GMPT ID:", gmptCode);
      ...
      if (axios.isAxiosError(error)) {
        if (error.response && error.response.status === 404) {
          console.error('No encontrado: El vehículo con el ID especificado no fue encontrado.');
        } else {
          console.error('Error fetching data:', error);
        }
      } else {
    ...
    const checkStoredGmptCode = async () => {
      try {
        ...
        if (storedGmptCode && storedGmptCode.trim() !== '') {
          console.log('GMPT ID encontrado en localStorage:', storedGmptCode);
  ...
  return (
    <>
      <div className="bg-teal-50 min-h-screen text-center text-lg w-full">
        ...
        <div className="w-full flex justify-center pb-2 mt-5 relative border border-teal-50 overflow-x-auto sm:rounded-lg ">
          <div className="bg-teal-50 w-1/2 m-auto">
            <div className="flex items-center justify-between py-5 px-8">
              <p className="border-l-2 border-teal-700 pl-3 text-teal-700 font-semibold">Equipment details</p>
              <div>
        ...
        <div className="w-full text-sm text-left rounded-md rtl:text-right text-gray-500 flex justify-center pb-4">
            ...
            <div>
              ...
              {results.length > 0 ? (
                results.map((result, index) => (
                  <div className="px-4 py-4 flex justify-evenly text-base border-x-2 border-b-2 border-gray-400 bg-white h-24" key={index}>
                    ...
                    <div className="text-center">
                      <label className="font-bold flex">Full Lockout Enabled: </label>
                      <span>{result.FullLockoutEnabled ? 'Yes' : 'No'}</span>
                    </div>
                  </div>
                ))
              ) : (
          ...
          <div className="w-1/3">
            <div>
              ...
              {results.length > 0 ? (
                results.map((result, index) => (
                  <div className="px-4 py-4 flex justify-around text-base border-x-2 border-b-2 border-gray-400 bg-white h-24" key={index}>
                    <div className="text-center">
                      <label className="font-bold flex">Idle Timeout Enabled: </label>
                      <span>{result.FullLockoutEnabled ? 'Yes' : 'No'}</span>
                    </div>
                    <div className="text-center">
                      <label className="font-bold flex">Idle Timeout Timer: </label>
                      <span>{result.IdleSetting?.IdleTimeoutTimer}</span>
                    </div>
                    <div className="text-center">
                      <label className="font-bold flex">Canbus: </label>
                      <span>{result.IdleSetting?.IsCanBus ? 'Yes' : 'No'}</span>
                    </div>
                  </div>
                ))
              ) : (
        ...
        <div className="w-full text-sm text-left rounded-md rtl:text-right text-gray-500 flex justify-center pb-4">
            ...
            <div>
              ...
              {results.length > 0 ? (
                results.map((result, index) => (
                  <div className="px-4 py-4 flex justify-evenly text-base border-x-2 border-b-2 border-gray-400 bg-white h-24" key={index}>
                    <div className="text-center">
                      <label className="font-bold flex">Driverbase/Timebase: </label>
                      <span>{result.ChecklistSetting?.DriverBaseOrTimebase}</span>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      <label className="font-bold flex">ChecklistTimeoutInSec: </label>
                      <span>{result.ChecklistSetting?.ChecklistTimeoutInSec}</span>
                    </div>
                  </div>
                ))
              ) : (
          ...
          results.map((result, index) => {
            ...
            const impactThreshold = result.FSSS_BASE * result.FSSSMULTI;

            console.log(impactThreshold);
            
            const blueImpactGForce = (G_FORCE_COEFFICIENT * Math.sqrt(impactThreshold * BLUE_IMPACT_COEFFICIENT)).toFixed(1);
            const amberImpactGForce = (G_FORCE_COEFFICIENT * Math.sqrt(impactThreshold * AMBER_IMPACT_COEFFICIENT)).toFixed(1);
            const redImpactGForce = (G_FORCE_COEFFICIENT * Math.sqrt(impactThreshold * RED_IMPACT_COEFFICIENT)).toFixed(1);

            return (
              <div className="px-4 py-4 flex justify-around text-base border-x-2 border-b-2 border-gray-400 bg-white text-sm h-24" key={index}>
                <div className="text-center">
                  <label className="font-bold flex">Blue Impact: </label>
                  <span>{blueImpactGForce}</span>
                </div>
                <div className="text-center">
                  <label className="font-bold flex">Amber Impact: </label>
                  <span>{amberImpactGForce}</span>
                </div>
                <div className="text-center">
                  <label className="font-bold flex">Red Impact: </label>
                  <span>{redImpactGForce}</span>
                  {/* <span>{redImpactThreshold}</span> */}
                </div>
                <div className="text-center">
                  <label className="font-bold flex">Red Impact Threshold: </label>
                  <span>{result.FSSS_BASE + "0"}</span>
                </div>
                <div className="text-center">
                  <label className="font-bold flex">Impact Lockout </label>
                  <span>{result?.IMPACT_LOCKOUT ? "Yes" : "No"}</span>
                </div>
              </div>
            );
          })
                    ...
                    {result.DriverList?.length > 0 ? (
                      <div className="h-64 overflow-y-auto border-x-2 border-b-2 border-gray-400 bg-white">
                        <div className="py-4 flex justify-center text-base font-bold text-xl">
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Fullname: </label>
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Site: </label>
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Department: </label>
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Facility Code: </label>
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Card Pin No: </label>
                          <label className="font-bold flex w-1/6 flex flex-col items-center text-lg">Weigand:</label>
                        </div>
                ...
                results.map((result, index) => (
                  <div key={index}>
                    {result.supervisors?.length > 0 ? (
                      <div className="h-64 overflow-y-auto border-x-2 border-b-2 border-gray-400 bg-white">
                        {result.supervisors.map((supervisor: Supervisor, supervisorIndex: number) => (
                          <div
                            key={supervisorIndex}
                            className="py-4 flex justify-center text-base"
                          >
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Fullname: </label>
                              <span>{supervisor.driver_name}</span>
                            </div>
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Site: </label>
                              <span>{supervisor.NAME}</span>
                            </div>
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Department: </label>
                              <span>{supervisor.DEPT_NAME}</span>
                            </div>
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Facility Code: </label>
                              <span>{supervisor.CARD_PREFIX}</span>
                            </div>
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Card Pin No: </label>
                              <span>{supervisor.DRIVER_ID}</span>
                            </div>
                            <div className="w-1/6 flex flex-col items-center">
                              <label className="font-bold flex">Weigand: </label>
                      ...
                      <div className="bg-white border-b hover:bg-gray-50">
                        <span className="px-6 py-4">No supervisors found</span>
                      </div>
                    )}
                  </div>
                ))
                ...
                results.map((result, index) => (
                  <div key={index}>
                    {result.PreopChecklists?.length > 0 ? (
                      <div className="h-64 overflow-y-auto border-x-2 border-b-2 border-gray-400 bg-white">
                        {result.PreopChecklists.map((checklist, checklistIndex) => (
                          <div
                            key={checklistIndex}
                            className="py-4 flex justify-center text-base"
                          >
                            <div className="w-1/5 flex flex-col items-center">
                              <label className="font-bold flex">Order: </label>
                              <span>{checklist.Order}</span>
                            </div>
                            <div className="w-1/5 flex flex-col items-center">
                              <label className="font-bold flex">Question: </label>
                              <span>{checklist.QUESTION}</span>
                            </div>
                            <div className="w-1/5 flex flex-col items-center">
                              <label className="font-bold flex">Type: </label>
                              <span>{checklist.Type}</span>
                              <span>Yes/no</span>
                            </div>
                            <div className="w-1/5 flex flex-col items-center">
                              <label className="font-bold flex">Expected Answer: </label>
                              <span>{checklist.ExpectedAnswer}</span>
                              <span>{checklist.EXP_ANS}</span>
                            </div>
                            <div className="w-1/5 flex flex-col items-center">
                              <label className="font-bold flex">Critical: </label>
                      ...
                      <div className="bg-white border-b hover:bg-gray-50">
                        <span className="px-6 py-4">No pre-op checklists found</span>
                      </div>
                    )}
                  </div>
                ))
      ...
      {showModal && <ModalCreateTicket handleModalTicket={handleShowModal} />}
      <ChatWidget />
    </>     
  );
};
