...
import { faBold, faImage, faItalic, faUnderline, faListUl,faLink,faUnlink ,faQuoteRight  } from '@fortawesome/free-solid-svg-icons';
...
const CommentList: React.FC<CommentListProps> = ({ comments, onAdd, onEdit, onDelete }) => {
  const [newCommentContent, setNewCommentContent] = React.useState('');
  const editor = useEditor({
    extensions: [StarterKit, Bold, Italic, Document, Paragraph, Text, Underline, Image, BulletList, ListItem, Link.configure({
      openOnClick: false,
      autolink: true,
      
    }), Blockquote, CharacterCount.configure({limit:4000}), ],
    ...
    <div className="p-4">
      ...
      <div className="comment-input-section">
        <div className="mt-1 flex items-center space-x-5 shadow-md pl-3 h-10">
          ...
          <button onClick={toggleLink} className={editor.isActive('link') ? 'is-active' : ''}>
...
>
  ...
  <div className="pt-2 space-y-4">
    {comments.map((comment) => (
      <Comment key={comment.IDComment} comment={comment} onEdit={onEdit} onDelete={onDelete} />
    ))}
  </div>
  <div>
